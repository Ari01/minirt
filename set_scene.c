...
/*   Updated: 2021/01/06 12:15:36 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		set_camera(char **split, t_rt *rt)
{
	...
	if (!set_coord(coord, &camera->position) || !set_coord(direction, &camera->direction)
		...
		return (0);
	camera->to_world_matrix[0] = new_vector4(1, 0, 0, 0);
	camera->to_world_matrix[1] = new_vector4(0, 1, 0, 0);
	camera->to_world_matrix[2] = new_vector4(0, 0, 1, 0);
	camera->to_world_matrix[3] = new_vector4(camera->position.x, camera->position.y, camera->position.z, 1);
